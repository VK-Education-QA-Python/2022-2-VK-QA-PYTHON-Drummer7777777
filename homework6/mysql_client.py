...
class MySqlClient:
    ...
    def create_table_count_request_by_type(self):
        if not sqlalchemy.inspect(self.engine).has_table('count_request_by_type'):
            BaseCountRequestByType.metadata.tables['count_request_by_type'].create(self.engine)
